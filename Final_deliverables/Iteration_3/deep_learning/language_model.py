...
DEFAULT_TRAINING_CONFIG = {'batch_size': 100,                    # the size of a mini-batch
                           ...
                           'epochs': 1,                         # the maximum number of epochs to run
...
class Model(object):
    ...
    def compute_loss_derivative(self, output_activations, expanded_target_batch):
        ...
        # loss_derivative = (activations.output_layer - expanded_target_batch)
        ###########################   YOUR CODE HERE  ##############################
    ...
    def back_propagate(self, input_batch, activations, loss_derivative):
        ...
        """ STEP-I :
            starting from the last layer compute the gradient for the weights
            for sigmoid unit  dy/dw = y(1-y) * x
            Trick :y(1-y)- express the derivative as a function of ouput itself , to make the calculations easy
            x is the state of the layer that is previous to the current layer."""
        hid_to_output_weights_grad = np.dot(loss_derivative.T,activations.hidden_layer)
        output_bias_grad = sum(loss_derivative, 2);
            
        embed_to_hid_weights_grad = np.dot(hid_deriv.T,activations.embedding_layer)
        hid_bias_grad = sum(hid_deriv, 2) 
...
def train(embedding_dim, num_hid, config=DEFAULT_TRAINING_CONFIG):
    ...
    for epoch in range(1,num_epochs + 1):
        ...
        for m, (input_batch, target_batch) in enumerate(get_batches(train_inputs, train_targets,
            ...
            cross_entropy = model.compute_loss(activations.output_layer, expanded_target_batch) / batch_size
